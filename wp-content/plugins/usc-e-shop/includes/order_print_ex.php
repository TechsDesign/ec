...
function usces_pdfSetFooter( $pdf, $data, $font, $usces_tax ) {
	...
	if( usces_is_tax_display() && 'all' == usces_get_tax_target() ) {
		...
		$pdf->SetXY( 142.9, $line_y );
		$pdf->MultiCell( 22.6, $lineheight, usces_conv_euc( $usces->get_currency( $usces_tax->tax_standard ) ), 0, 'R' );
		...
		$pdf->SetXY( 142.9, $line_y );
		$pdf->MultiCell( 22.6, $lineheight, usces_conv_euc( $usces->get_currency( $usces_tax->tax_reduced ) ), 0, 'R' );
		$pdf->SetXY( 163.5, $line_y );
		$pdf->MultiCell( 5, $lineheight, usces_conv_euc( ')' ), 0, 'C' );
	}
