...
<?php
	...
	function download_orderdetail_list() {
		...
		if( usces_is_tax_display() && 'products' == usces_get_tax_target() ) {
			...
			if( usces_is_reduced_taxrate() ) {
				if( isset( $_REQUEST['check']['subtotal_standard'] ) ) $line .= $th_h.sprintf( __( "Applies to %s%%", 'usces' ), $usces_tax->tax_rate_standard ).$th_f;
				if( isset( $_REQUEST['check']['tax_standard'] ) ) $line .= $th_h.sprintf( __( "%s%% consumption tax", 'usces' ), $usces_tax->tax_rate_standard ).$th_f;
				if( isset( $_REQUEST['check']['subtotal_reduced'] ) ) $line .= $th_h.sprintf( __( "Applies to %s%%", 'usces' ), $usces_tax->tax_rate_reduced ).$th_f;
