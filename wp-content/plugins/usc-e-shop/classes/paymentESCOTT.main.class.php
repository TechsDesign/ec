...
<?php			endif;
	...
	public function done_3dsecure(  ) {
		...
		$algo = 'AES-128-CBC';
		$key = $acting_opts['sec3d_encKey'];
		$iv = $acting_opts['sec3d_iv'];
...
?>
			...
			<div class="wait_message" style="text-align: center; margin-top: 100px;"><?php esc_html_e( __( 'Hold on for a while, please.', 'uesces' ) ); ?></div>
			</form>
			</body>
			</html>
<?php
	 ...
	 * @return -
	 */
	public function certification_3dsecure( $params, $post_data ) {
	
		$post_data2['escott3d'] = 'done';//フラグを立てる
		$_SESSION['post3d'] = $post_data;//POSTを保存しておく
		$token = ( isset( $_POST['token'] ) ) ? trim( $_POST['token'] ) : '';
		$params['OperateId'] = '3Secure';
		$params['ProcNo'] = usces_rand( 7 );//適当な数値7桁
		$params['RedirectUrl'] = add_query_arg( $post_data2, USCES_CART_URL );

		//クイックの時はトークンを入れない
		if( !isset($params['KaiinId']) || empty($params['KaiinId']) ){
				$params['Token'] = $token;
		}
	...
	public function acting_processing( $acting_flg, $post_query ) {
		...
		if( $this->acting_flg_card == $acting_flg ) {
			if( 'on' == $acting_opts['card_activate'] || 'token' == $acting_opts['card_activate'] ) {
				...
				//3Dセキュア認証の結果を受ける時はスルー
				//3Dの前に一度通過している
				if( !isset( $post_data['done3d'] ) ) {
				...
				//3Dセキュア認証の結果を受ける
				if( isset($_REQUEST['done3d']) ){

					usces_p('OK');
					usces_p($post_data);
					usces_p($_REQUEST);
					
					//認証が通らなかった(1)場合はエラー画面
					if( isset($post_data['SecureResultCode']) && 1 == $post_data['SecureResultCode'] ){
					...
					//3DセキュアがOK（0）だった時のみ、3D情報を追加
					}elseif( isset($post_data['SecureResultCode']) && 0 == $post_data['SecureResultCode'] ){
					
						$params['param_list']['EncodeXId3D'] = $post_data['EncodeXId3D'];
						$params['param_list']['TransactionId3D'] = $post_data['TransactionId3D'];
						$params['param_list']['MessageVersionNo3D'] = $post_data['MessageVersionNo3D'];
						$params['param_list']['TransactionStatus3D'] = $post_data['TransactionStatus3D'];
						$params['param_list']['CAVVAlgorithm3D'] = $post_data['CAVVAlgorithm3D'];
						$params['param_list']['ECI3D'] = $post_data['ECI3D'];
						$params['param_list']['CAVV3D'] = $post_data['CAVV3D'];
						
					}else{
						//SecureResultCodeが2以上の場合は、そのまま通過
					}
					...
					if( 'on' == $acting_opts['sec3d_activate'] ){
						$results = $this->certification_3dsecure( $param_list, $post_data );
					}
				}
