...
class WELCARTPAY_SETTLEMENT extends ESCOTT_MAIN
{
	...
	public function initialize_data() {
		...
		if( !isset( $options['acting_settings'] ) || !isset( $options['acting_settings']['welcart'] ) ) {
			...
			$options['acting_settings']['welcart']['sec3d_encKey'] = ( isset( $options['acting_settings']['welcart']['sec3d_encKey'] ) ) ? $options['acting_settings']['welcart']['sec3d_encKey'] : '';
			$options['acting_settings']['welcart']['sec3d_iv'] = ( isset( $options['acting_settings']['welcart']['sec3d_iv'] ) ) ? $options['acting_settings']['welcart']['sec3d_iv'] : '';
...
jQuery( document ).ready( function( $ ) {
	...
	$( document ).on( "change", ".card_activate_welcart", function() {
		...
		if( "off" == $( ".sec3d_activate_welcart:checked" ).val() ) {
			$( ".card_sec3d_welcart" ).css( "display", "none" );
		}
	});
...
<?php
	...
	public function settlement_update() {
		...
		$options['acting_settings']['welcart']['sec3d_encKey'] = ( isset( $_POST['sec3d_encKey'] ) ) ? trim($_POST['sec3d_encKey']) : '';
		$options['acting_settings']['welcart']['sec3d_iv'] = ( isset( $_POST['sec3d_iv'] ) ) ? trim($_POST['sec3d_iv']) : '';
		...
		if( '' == $this->error_mes ) {
			...
			if( ( 'on' == $options['acting_settings']['welcart']['card_activate'] || 'link' == $options['acting_settings']['welcart']['card_activate'] || 'token' == $options['acting_settings']['welcart']['card_activate'] ) || 
				'on' == $options['acting_settings']['welcart']['conv_activate'] || 'on' == $options['acting_settings']['welcart']['atodene_activate'] ) {
...
?>
	...
	<form action="" method="post" name="welcart_form" id="welcart_form">
		...
		<table class="settle_table">
			...
			<tr id="ex_foreign_activate_welcart" class="explanation card_welcart"><td colspan="2"><?php _e( 'Foreign exchange settlement, only two VISA and MasterCard card companies are available.<br />If switching to Yen card is done during operation, the member information registered in e-SCOTT will be invalid.', 'usces' ); ?></td>
			</tr>-->
			<tr class="card_welcart">
			...
			<tr class="card_sec3d_welcart card_welcart">
			...
			<tr id="ex_sec3d_encKey" class="explanation card_sec3d_welcart card_welcart"><td colspan="2"><?php _e( "3D secure encryption key (single-byte alphanumeric characters only) issued by e-SCOTT.", 'usces' ); ?></td></tr>
			<tr class="card_sec3d_welcart card_welcart">
			...
			<tr id="ex_sec3d_iv" class="explanation card_sec3d_welcart card_welcart"><td colspan="2"><?php _e( "3D secure initialization vector (single-byte alphanumeric characters only) issued by e-SCOTT.", 'usces' ); ?></td></tr>
			<tr class="card_welcart">
...
<?php
	...
	public function set_uscesL10n( $l10n, $post_id ) {
		...
		if( $usces->is_member_page( $_SERVER['REQUEST_URI'] ) && ( isset( $_GET['page'] ) && ( 'member_register_settlement' == $_GET['page'] || 'member_update_settlement' == $_GET['page'] ) ) ) {
			$acting_opts = $this->get_acting_settings();
			if( isset( $acting_opts['card_activate'] ) && 'token' == $acting_opts['card_activate'] ) {
				$l10n .= "'front_ajaxurl': '".USCES_SSL_URL."',\n";
