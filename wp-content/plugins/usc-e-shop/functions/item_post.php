...
function order_item_ajax(){
	...
	switch ( $_POST['mode'] ) {
		...
		case 'getmember':
			...
			break;
		case 'recalculation':
			$res = usces_order_recalculation( $_POST['order_id'], $_POST['mem_id'], $_POST['post_ids'], $_POST['skus'], $_POST['prices'], $_POST['quants'], $_POST['cart_ids'], $_POST['upoint'], $_POST['shipping_charge'], $_POST['cod_fee'], $_POST['discount'], $_POST['change_taxrate'] );
			break;
		case 'recalculation_reduced':
			$res = usces_order_recalculation_reduced( $_POST['order_id'], $_POST['mem_id'], $_POST['post_ids'], $_POST['prices'], $_POST['quants'], $_POST['cart_ids'], $_POST['upoint'], $_POST['shipping_charge'], $_POST['cod_fee'], $_POST['discount_standard'], $_POST['discount_reduced'], $_POST['change_taxrate'] );
			break;
...
function usces_order_recalculation( $order_id, $mem_id, $post_id, $skus, $price, $quant, $cart_id, $use_point, $shipping_charge, $cod_fee, $discount = 0, $change_taxrate ) {
	global $usces;
	...
	if( 'activate' == $condition['membersystem_state'] && 'activate' == $condition['membersystem_point'] && !empty( $meminfo['ID'] ) ) {
		if( $condition['display_mode'] == 'Promotionsale' ) {
			if( $condition['campaign_privilege'] == 'discount' ) {
				foreach( $cart as $cart_row ) {
...
function usces_order_recalculation_reduced( $order_id, $mem_id, $post_id, $price, $quant, $cart_id, $use_point, $shipping_charge, $cod_fee, $discount_standard = 0, $discount_reduced = 0, $change_taxrate ) {
	...
	for( $i = 0; $i < $post_id_count; $i++ ) {
		if( $post_id[$i] ) {
			$ordercart = usces_get_ordercartdata_row( $cart_id[$i] );
			$applicable_taxrate = $usces_tax->get_ordercart_applicable_taxrate( $cart_id[$i], $post_id[$i], $ordercart['sku_code'] );
			$cart[] = array(
				'post_id'=>$post_id[$i],
