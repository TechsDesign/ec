...
function usces_order_confirm_message($order_id) {
	...
	if( usces_is_tax_display() && $reduced_taxrate ) {
		...
		$meisai .= sprintf( __( "Applies to %s%%", 'usces' ), $usces_tax->tax_rate_standard ) . " : " . usces_crform( $usces_tax->subtotal_standard, true, false, 'return' ) . "\r\n";
		$meisai .= sprintf( __( "%s%% consumption tax", 'usces' ), $usces_tax->tax_rate_standard ) . " : " . usces_crform( $usces_tax->tax_standard, true, false, 'return' ) . "\r\n";
		$meisai .= sprintf( __( "Applies to %s%%", 'usces' ), $usces_tax->tax_rate_reduced ) . " : " . usces_crform( $usces_tax->subtotal_reduced, true, false, 'return' ) . "\r\n";
		$meisai .= sprintf( __( "%s%% consumption tax", 'usces' ), $usces_tax->tax_rate_reduced ) . " : " . usces_crform( $usces_tax->tax_reduced, true, false, 'return' ) . "\r\n";
		$meisai .= usces_mail_line( 2, $data['order_email'] );//--------------------
