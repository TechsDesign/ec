...
function usces_confirm_tax( $out = '' ) {
	...
	if( $usces->is_reduced_taxrate() ) {
		...
		$tax  = '( '.sprintf( __( "Applies to %s%%", 'usces' ), $usces->options['tax_rate'] ).'&nbsp;&nbsp;'.usces_crform( $usces_tax->subtotal_standard, true, false, 'return' ).'&nbsp;&nbsp;';
		$tax .= sprintf( __( "%s%% consumption tax", 'usces' ), $usces->options['tax_rate'] ).'&nbsp;&nbsp;'.usces_crform( $usces_tax->tax_standard, true, false, 'return' ).')<br />';
		$tax .= '( '.sprintf( __( "Applies to %s%%", 'usces' ), $usces->options['tax_rate_reduced'] ).'&nbsp;&nbsp;'.usces_crform( $usces_tax->subtotal_reduced, true, false, 'return' ).'&nbsp;&nbsp;';
		$tax .= sprintf( __( "%s%% consumption tax", 'usces' ), $usces->options['tax_rate_reduced'] ).'&nbsp;&nbsp;'.usces_crform( $usces_tax->tax_reduced, true, false, 'return' ).')';
	} else {
		$tax  = '';
	}
